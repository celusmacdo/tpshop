...
class AdminuserController extends PublicController {
    ...
    public function index(){
		...
		$this->assign('title','麦斯威尔咖啡商城-管理员列表');
    ...
    public function add(){
		if(IS_POST){
			...
			$this->assign('title','麦斯威尔咖啡商城-管理员添加');
			$this->display();
		}
    }
		//管理员编辑
    public function update(){
		if(IS_POST){
			...
			// dump($rs);exit;
			$city=M('Region')->where('region_id='.$rs['city'])->find();
			$district=M('Region')->where('region_id='.$rs['district'])->find();
						
			$this->assign('province',D('Region')->getRegion(1));
			$this->assign('city',D('Region')->getRegion(2,$city['parent_id']));
			$this->assign('district',D('Region')->getRegion(3,$district['parent_id']));
			// dump($rs);exit;
			$this->assign('title','麦斯威尔咖啡商城-管理员修改');
			$this->assign('rs',$rs);
			$this->display();
		}
    }
