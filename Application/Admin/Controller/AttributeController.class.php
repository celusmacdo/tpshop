...
class AttributeController extends PublicController {
	//产品属性列表页
	    public function index(){
			...
			$perpage=isset($_GET['perpage'])?$_GET['perpage']:13;
			$rs = D(CONTROLLER_NAME)->showData($perpage);
			$this->assign('title','麦斯威尔咖啡商城-产品属性列表');
	    ...
	    public function add(){
			if(IS_POST){
				...
				$rs1=  D('Attribute')->showData(100000000);
				// dump($rs1["result"]);exit;
				$this->assign('attribute',$rs1['result']);
				// $this->assign('province',D('Region')->getRegion(1));
				$this->assign('title','麦斯威尔咖啡商城-产品属性添加');
				$this->display();
			}
	    }
		//产品属性编辑
	    public function update(){
			if(IS_POST){
				...
				// dump($rs);exit;
				$this->assign('title','麦斯威尔咖啡商城-产品属性修改');
				$this->assign('rs',$rs);
				$this->display();
			}
	    }
